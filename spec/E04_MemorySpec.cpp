...
namespace spec
{
    ...
    public:
		...
		void TestprimeFactorsSmall(){
			...
			Assert::AreEqual(true, areEqualArrays(primes->list, ans, 2), L"TestprimeFactorsSmall() failed", 1, 2);
			primes = primeFactors(75);
			int ans[] = { 3, 5, 5 };
			Assert::AreEqual(3, primes->count, L"TestprimeFactorsSmall() failed", 1, 2);
			Assert::AreEqual(true, areEqualArrays(primes->list, ans, 3), L"TestprimeFactorsSmall() failed", 1, 2);
			
			primes = primeFactors(96);
			int ans[] = { 2, 2, 2, 2, 2, 3 };
			Assert::AreEqual(6, primes->count, L"TestprimeFactorsSmall() failed", 1, 2);
			Assert::AreEqual(true, areEqualArrays(primes->list, ans, 6), L"TestprimeFactorsSmall() failed", 1, 2);
		}
		...
		void TestprimeFactorsMedium(){
			...
			int ans[] = { 2, 5, 5, 5, 7, 11 };
			Assert::AreEqual(6, primes->count, L"TestprimeFactorsMedium() failed", 1, 2);
			Assert::AreEqual(true, areEqualArrays(primes->list, ans, 6), L"TestprimeFactorsMedium() failed", 1, 2);
			primes = primeFactors(31247);
			int ans[] = { 31247 };
			Assert::AreEqual(1, primes->count, L"TestprimeFactorsMedium() failed", 1, 2);
			Assert::AreEqual(true, areEqualArrays(primes->list, ans, 1), L"TestprimeFactorsMedium() failed", 1, 2);

			primes = primeFactors(78057);
			int ans[] = { 3, 3, 3, 7, 7, 59 };
			Assert::AreEqual(6, primes->count, L"TestprimeFactorsMedium() failed", 1, 2);
			Assert::AreEqual(true, areEqualArrays(primes->list, ans, 6), L"TestprimeFactorsMedium() failed", 1, 2);
		}
		...
		void TestprimeFactorsLarge(){
			...
			int ans[] = { 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2 };
			Assert::AreEqual(30, primes->count, L"TestprimeFactorsLarge() failed", 1, 2);
			Assert::AreEqual(true, areEqualArrays(primes->list, ans, 30), L"TestprimeFactorsLarge() failed", 1, 2);
			primes = primeFactors(26165843);
			int ans[] = { 11, 71, 33503 };
			Assert::AreEqual(3, primes->count, L"TestprimeFactorsLarge() failed", 1, 2);
			Assert::AreEqual(true, areEqualArrays(primes->list, ans, 3), L"TestprimeFactorsLarge() failed", 1, 2);

			primes = primeFactors(691979840);
			int ans[] = { 2, 2, 2, 2, 2, 2, 5, 23, 149, 631 };
			Assert::AreEqual(10, primes->count, L"TestprimeFactorsLarge() failed", 1, 2);
			Assert::AreEqual(true, areEqualArrays(primes->list, ans, 10), L"TestprimeFactorsLarge() failed", 1, 2);
		}
    };
}
